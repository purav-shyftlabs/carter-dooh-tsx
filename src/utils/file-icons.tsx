...
export const getFileIcon = (contentType: string, filename?: string, size: 'normal' | 'small' = 'small'): React.ReactElement => {
  ...
  if (contentType.startsWith('image/') || ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'svg', 'webp', 'ico'].includes(extension)) {
    ...
    return <FileZip size={size === 'small' ? 18 : 42} color="#F59E0B" weight="regular" />; // Orange for archives
  } else if (contentType.includes('presentation') || 
             contentType.includes('powerpoint') ||
             ['ppt', 'pptx', 'odp'].includes(extension)) {
    return <FilePpt size={size === 'small' ? 18 : 42} color="#EA580C" weight="regular" />; // Orange for presentations
  } else if (contentType.includes('document') ||
             contentType.includes('word') ||
             ['doc', 'docx', 'odt', 'rtf'].includes(extension)) {
    return <FileDoc size={size === 'small' ? 18 : 42} color="#2563EB" weight="regular" />; // Blue for Word documents
  } else if (contentType.startsWith('text/') || 
             ['txt', 'md', 'json', 'xml', 'html', 'css', 'js', 'ts'].includes(extension)) {
    return <FileText size={size === 'small' ? 18 : 42} color="#1E40AF" weight="regular" />; // Dark blue for text files
  } else {
    return <File size={size === 'small' ? 18 : 42} color="#6B7280" weight="regular" />; // Gray for unknown files
  }
};
